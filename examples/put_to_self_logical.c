...
/*
 * This example shows a PUT data transfer to ourselves with a logical interface
 */
...
int main(void)
{
	...
	ptl_le_t le;
	ptl_md_t md;
	char data[] = "Hello, put_to_self logical !";
	...
	/* set the mapping from our logical id (rank) to our physical id (nid and pid)*/
	ret = PtlSetMap(nih, 1, mapping);
	if (ret != PTL_OK) {
	...
	ret = PtlPut(mdh, 0, sizeof(data), PTL_ACK_REQ, rank0, pti, 0, 0, NULL, 0);
	if (ret != PTL_OK) {

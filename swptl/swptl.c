 ...
 */
int ptl_atsize(ptl_datatype_t atype)
{
	switch (atype) {
	case PTL_INT8_T:
	case PTL_UINT8_T:
 ...
 */
int swptl_eq_getev(struct swptl_eq *eq, ptl_event_t *ev)
{
	struct swptl_ev *e;
		   ...
		   void *uptr)
{
	...
	memset(&dummy, 0, sizeof(ptl_process_t));
 ...
 */
void swptl_ct_get(struct swptl_ct *ct, ptl_ct_event_t *rval)
{
	*rval = ct->val;
}
...
int swptl_func_ni_init(struct swptl_dev *dev, unsigned int flags,
		       const ptl_ni_limits_t *desired_lim, ptl_ni_limits_t *actual_lim,
...
int swptl_func_setmap(struct swptl_ni *ni, ptl_size_t size, const ptl_process_t *map)
{
	...
	memcpy(p, map, size * sizeof(ptl_process_t));
...
int swptl_func_getmap(struct swptl_ni *ni, ptl_size_t size, ptl_process_t *map, ptl_size_t *retsize)
{
	...
	*retsize = ni->mapsize;
	if (size > ni->mapsize)
		size = ni->mapsize;
	memcpy(map, ni->map, size * sizeof(ptl_process_t));
...
int swptl_func_pte_enable(struct swptl_ni *ni, ptl_pt_index_t index, int enable)
{
	struct swptl_pte *pte = ni->pte[index];
...
int swptl_func_trigget(struct swptl_md *mdh, ptl_size_t loffs, ptl_size_t len, ptl_process_t dest,
		       ptl_pt_index_t index, ptl_match_bits_t bits, ptl_size_t roffs, void *uptr,
		       struct swptl_ct *ct, ptl_size_t thres)
{
	struct swptl_ni *ni = mdh->ni;
	int rc;
